...
- **Echo mode** — Records user actions (keystrokes, commands, selections) into a replayable workbook (`.gecho.json`)
- **GIF mode** — Captures the VS Code window as a GIF via ffmpeg
- **Combined** — Replay a workbook while capturing the screen to produce a deterministic GIF
...
```
src/types/        — Workbook + recording types (discriminated unions on `type` field)
...
workbooks/        — Example .gecho.json workbooks
...
5. **Workbook format changes** — Any addition or change to `.gecho.json` step types requires a version bump + migration function.
...
Workbooks are `.gecho.json` files validated against `schemas/gecho-v1.schema.json` (JSON Schema draft-07). The schema uses `oneOf` for the step discriminated union. Current step types: `type`, `command`, `key`, `select`, `wait`, `openFile`, `paste`, `scroll`.
...
docs: Update workbook format reference with paste step
